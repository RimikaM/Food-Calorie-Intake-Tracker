...
export DATABASE_URL="postgresql://food_calorie_user:your-password@localhost:5432/food_calorie_intake"
```
...
This repo includes `render.yaml` for one-click setup with a free web service and free PostgreSQL.
...
3. Render reads `render.yaml` and creates:
     - Web service: `food-calorie-intake`
     - PostgreSQL database: `food-calorie-intake-db`

### 3. Configure environment variables

- `SECRET_KEY` is auto-generated by Render from `render.yaml`.
- Set `USDA_FDC_API_KEY` manually in Render web service settings.
...
- On startup, the app runs `init_db()` and creates tables automatically in PostgreSQL.

### 5. Verify

- Open your Render app URL.
- Register a new user and add one food entry.
- Confirm data persists after restarting/redeploying.

Notes:
- Free instances may spin down when idle.
- First request after idle can take longer.
